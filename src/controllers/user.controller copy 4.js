    ...
    file.mv(uploadPath, async (err) => {
      ...
      try {
        ...
        return res.status(201).json({
          message: "Excel processed and split successfully!",
          headers,
          sampleRow: values[0],
          newFilePath,
        });
...
async function createEmpSheet(jsonData1, jsonData2) {
  try {
    ...
    const headers = Object.keys(jsonData1[0]);
    const values = jsonData1.map((obj) => headers.map((h) => obj[h]));
